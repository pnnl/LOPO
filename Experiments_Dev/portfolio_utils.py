...
def gen_portfolio_lto_data(n_dim,ntrain,nvalid,ntest):
    ...
    filename = Path("./portfolio_data/portfolio_var{}_train{}_valid{}_test{}.npz".format(n_dim, ntrain, nvalid, ntest))
    if filename.exists():
        ...
        p_train = p[:, :ntrain]
        train_Y,t = solve_convexqp(2*Q, p_train, A, G, h, x)
        ...
        valid_Y,t = solve_convexqp(2*Q, p_valid, A, G, h, x)
        ...
        test_Y,t  = solve_convexqp(2*Q, p_test,  A, G, h, x)
        ...
        with open(filename, 'wb') as f:
            np.savez_compressed(f, Q=Q, A=A, G=G, h=h, x=x, trainX=p_train.T, validX=p_valid.T, testX=p_test.T, trainY=train_Y, validY=valid_Y, testY=test_Y)# fix the solver for testY
...
# CONVEX QP PROBLEM
###################################################################
def solve_convexqp(Q, p, A, G, h, X, tol=1e-12, max_iter=10000):
    #print('running osqp')
    ...
    for pi in p.T:
        ...
        my_u = np.hstack([Xi, h])
        solver.setup(P=csc_matrix(Q), q=pi, A=csc_matrix(my_A), l=my_l, u=my_u, verbose=False, eps_prim_inf=tol, max_iter=max_iter)
        start_time = time.time()
        results = solver.solve()
        end_time = time.time()
        total_time += (end_time - start_time)
        avg_obj += (results.x @ Q) @ results.x + pi @ results.x
...
def cvx_qp(N,Q):
    ...
    problem  = cp.Problem(cp.Minimize(  c @ x  + cp.quad_form(x,Q)   ),  constraints)
