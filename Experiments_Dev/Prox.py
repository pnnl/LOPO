...
class ModularQuadraticObjectiveLinearEqualityComposition(torch.nn.Module):


    def __init__(self,f,F,metric,JF_fixed = False,n_dim = None, parm_dim = None, precision = torch.float32, gamma=2.0):
        '''
        :param f: (functorch compatible function) a parameterized function f with input of the form (x,parms) where parms is a tensor that matches the batch of x, and has last dim parm features.
                  f is defined unbatched, the method will call vmap, to "raise" f to batch dim
        ...
        self.JF_fixed = JF_fixed
        if self.JF_fixed == True:
            jx = torch.zeros((1,n_dim),dtype = precision)
            jp = torch.zeros((1,parm_dim),dtype = precision)
            JFx = self.JF(jx,jp)
            Qc, Rc  = torch.linalg.qr(torch.transpose(JFx,1,2),mode = 'complete')
            null_dim = Qc.shape[-1] - Rc.shape[-1]
            Rr = Rc[:,:-null_dim,:]
    ...
    def forward(self, x, parms, H_factors, J_factors):

        x = x.to(self.precision)
        parms = parms.to(self.precision)

        if self.JF_fixed == False:
            Rr,Qr,Qn,JFx = J_factors
    ...
    def prefactor(self, x, parms):
        ...
        ### Compute the gradient step and Projection Operation
        Pm = vmap(self.metric)(x.float(),parms.float()).to(self.precision)
        Md =  (self.gamma/2)*Pm + Hf
        QTMdQ = torch.bmm(torch.bmm(torch.transpose(Qn,1,2),Md),Qn)
...
class EqualityConstrainedQuadratic(torch.nn.Module):
    ...
    def __init__(self,f,F,metric,JF_fixed = False,n_dim = None, parm_dim = None, slack_mode = False, n_ineq = 0, gamma=2.0):
        '''
        :param f: (functorch compatible function) a parameterized function f with input of the form (x,parms) where parms is a tensor that matches the batch of x, and has last dim parm features.
                  f is defined unbatched, the method will call vmap, to "raise" f to batch dim
...
"""
An inefficient implementation of the projection onto
semidefinite cone X>>0
w.r.t. variable metric P, i.e.

minimize p @ (x^T x)
s.t.          X>>0

Where x,p are the flattened vectors representing X and P

In absence of an analytical solution to this projection,
we solve the above problem by unrolling Projected Gradient Descent,
which utilizes the solution of the typical Euclidean norm projection as
an eigenvalue problem at each step.
"""
class SemidefiniteConePGD(torch.nn.Module):


    def __init__(self,metric, n_steps = 100, stepsize = 0.05, precision = torch.float32):
    ...
    def semidef_cone_eig_prox(self,X):
    ...
    """
    No parameters, the constraint is simply X>>0
    X and parms should have shape (batch,N,N)
    """
    def forward(self, X, parms):
        ...
        P = vmap(self.metric)(X.view(batch,-1).float(),parms.view(batch,-1).float()).to(self.precision)
        Z = X
        for _ in range(self.n_steps):
            grad = (  2*P.view(batch,-1)*(X.view(batch,-1) - Z.view(batch,-1))  ).view(batch,N,N)
            X = vmap(self.semidef_cone_eig_prox)( X - alpha*grad )
        return X
