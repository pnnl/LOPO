...
"""
optimizer = torch.optim.AdamW(problem.parameters(), lr=1e-4)
...
'''
def f_obj(x,parms):
    return torch.pow( (1 - x[0] ),2) + parms[0]*torch.pow( x[1] - torch.pow(x[0],2), 2)

'''
# DEFINE THE CONSTRAINTS
'''
def F_ineq(x,parms):
    fq_l = torch.pow(parms[1]/2,2)  - torch.pow(x[0],2) - torch.pow(x[1],2)
    fq_u = torch.pow(x[0],2) + torch.pow(x[1],2) - torch.pow(parms[1],2)
    fxy =  x[1] - x[0]
    return torch.stack((fq_l,fq_u,fxy))
...
solver = dr.DRSolver(
    f_obj = f_obj, 
    F_ineq = F_ineq,
    x_dim = 2, n_ineq = 3, order = 'second',num_steps = num_steps)
...
DR_correction = Node(solver,['x_predicted','xi'],['x'])
...
#x_nm = model_out['test_x_predicted'][0, 0].detach().numpy()
#y_nm = model_out['test_x_predicted'][0, 1].detach().numpy()
...
p_eval = p_samples.detach().numpy()
a_eval = a_samples.detach().numpy()

# Solve via solver
t = time.time()
x_solver, y_solver = [], []
for i in range(0, n_samples):
    prob, x, y = NLP_param( a_samples[i].numpy()[0],p_samples[i].numpy()[0], opti_silent=True)
    sol = prob.solve()
